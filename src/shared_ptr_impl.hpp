...
public:
    ...
    int *get_count_ptr() const { return count; }
...
public:
    ...
    void incr() const { base_ptr->incr(); }
    void decr() const { base_ptr->decr(); }
...
public:
    ...
    explicit shared_ptr(T *p)
        : base_ptr(new shared_ptr_base<T>(p))
    {}
    ...
    explicit shared_ptr(const shared_ptr &ptr)
    ...
    explicit shared_ptr(U *ptr, Deleter d)
        : base_ptr(new shared_ptr_base<U>(ptr, new int(1), d))
    {}

    template<class Deleter>
    explicit shared_ptr(std::nullptr_t, Deleter d)
        : base_ptr(new shared_ptr_base<T>(nullptr, new int(1), d))
    {}
    ...
    void swap(shared_ptr &other) { std::swap(this->base_ptr, other.base_ptr); }

    template<typename U>
    void reset(U *ptr)
    {
        if (ptr == nullptr || this->get() == ptr)
            return;
        base_ptr->decr();
    ...
    ~shared_ptr() { base_ptr->decr(); }
    template<typename U>
    shared_ptr(const shared_ptr<U> &ptr)
        : base_ptr(new shared_ptr_base<T>(static_cast<T *>(ptr.base_ptr->res_get()),
                                          ptr.get_count_ptr()))
    {
        ptr.incr();
    }

    unsigned long use_count() const { return base_ptr->use_count(); }
    bool unique() const { return (base_ptr->use_count() == 1); }
    T &operator*() const { return *(base_ptr->res_ptr); }

    T *operator->() const { return base_ptr->res_ptr; }
    T *get() const { return base_ptr->res_get(); }
    operator bool() const
    {
        if (*this == nullptr)
            return false;
    ...
    //不符合标准
    int *get_count_ptr() const { return base_ptr->get_count_ptr(); }
    template<typename U>
...
public:
    ...
    explicit weak_ptr(const weak_ptr &ptr)
        : base_ptr(ptr.base_ptr)
    {
        base_ptr->incr_weakRef();
    }
    explicit weak_ptr(const shared_ptr<T> &ptr)
        : base_ptr(ptr.base_ptr)
    {
        base_ptr->incr_weakRef();
    }
    explicit weak_ptr(weak_ptr &&ptr)
        : base_ptr(ptr.base_ptr)
    {
