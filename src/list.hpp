...
struct ListNode
{
    ...
    ListNode(const T &val, ListNode *prev = nullptr, ListNode *succ = nullptr)
        : nValue(val)
    {
        nPrev = prev;
        nNext = succ;
...
public:
    explicit ListIterator(node_type *ptr = nullptr)
        : nNode(ptr)
    {}
    ListIterator(const ListIterator &other)
        : nNode(const_cast<node_type *>(other.nNode))
    {}
...
template<typename T, typename Pointer, typename Reference>
bool operator==(const ListIterator<T, Pointer, Reference> &a,
...
public: // for test only
    ...
    template<typename Compare>
    ListNode<T> *_Merge_sort(ListNode<T> *head, Compare comp)
    {
        if (head == nullptr || head->nNext == nullptr)
            return head;
        //fast and slow
...
{
    if (pos == end())
        return iterator(nullptr); //undefined behaviour
...
template<typename T, typename Alloc>
template<class... Args>
typename list<T, Alloc>::reference list<T, Alloc>::emplace_front(Args &&... args)
{
    auto p = dataAlloc.allocate();
...
template<typename T, typename Alloc>
template<class... Args>
typename list<T, Alloc>::reference list<T, Alloc>::emplace_back(Args &&... args)
{
    auto p = dataAlloc.allocate();
    dataAlloc.construct(p, pdstl::move(ListNode<T>(pdstl::forward<Args>(args)...)));
    ++_size;
