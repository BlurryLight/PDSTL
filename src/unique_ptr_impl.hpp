...
public:
    ...
    explicit unique_ptr()
        : res_ptr(nullptr)
    {}
    explicit unique_ptr(std::nullptr_t)
        : res_ptr(nullptr)
    {}
    ...
    explicit unique_ptr(T *p)
        : res_ptr(p)
    {}
    explicit unique_ptr(unique_ptr &&ptr)
        : res_ptr(ptr.res_ptr)
    {
        ptr.res_ptr = nullptr;
    }
    ...
    T *get() const { return res_ptr; }

    void swap(unique_ptr &other) { std::swap(res_ptr, other.res_ptr); }
    void reset()
    {
        destory();
        res_ptr = nullptr;
    ...
    T &operator*() const { return *(res_ptr); }

    T *operator->() const { return res_ptr; }

    operator bool() const { return (res_ptr != nullptr); }
};
